...
def bubble_sort(numbers, debug=False):
...
if __name__ == '__main__':
    import random
    with Timer():
        bubble_sort([5, 9, 3, 1, 2, 8, 4, 7, 6], debug=True)
    with Timer():
        bubble_sort([random.randint(1, 10) for _ in range(100)])
    with Timer():
        bubble_sort([random.randint(1, 10) for _ in range(10000)])
